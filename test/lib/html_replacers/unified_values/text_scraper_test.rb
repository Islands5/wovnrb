...
class UnifiedValuesTextScraperTest < ActiveSupport::TestCase
  def assert_shared_fixture(base_name, ignored_classes = Set.new)
    html = File.read("shared_fixtures/unified_values/#{base_name}_actual.html")
    expected_values = JSON.parse(File.read("shared_fixtures/unified_values/#{base_name}_expected.json"))
